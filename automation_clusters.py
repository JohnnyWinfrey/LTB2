...
class SLIM(QObject):
    ...
    def _edgeLP(self):
        ...
        for x in range(0, 41, 1):           # 0–20 mm in 0.5 mm steps
            if not self.worker._is_running:
                break
            for angleStep in range(0, 91, 10):   # PSG linear polarization states
                if not self.worker._is_running:
                    break
                for analyzeStep in range(-45, 91, 45):  # FIX 4: fixed typo 'anaylzeStep'
                    if not self.worker._is_running:
                        break
                    data = self.slimScan(
                        P1=angleStep, R1=angleStep,
    ...
    def _cali(self):
        # FIX 5: removed blocking input() call — emitting a signal instead.
        # Connect SLIMBackend.calibrationPause to a UI dialog in QML,
        # then call SLIMBackend.resumeCalibration() from the dialog button.
        ...
        for i in range(11, 3961, 11):
            ...
            data = self.slimScan(P1=0, R1=i, R2=i * 5, P2=0)
            cal_data.append(data)  # FIX 3: append, not extend
            print(f"Collection at R1: {i}  R2: {i * 5}")
 
            if i == 1980:
                # Pause the worker thread and wait for UI confirmation
                print("Calibration paused at midpoint — call resumeCalibration() to continue.")
                while self._cali_paused and self.worker._is_running:
                    time.sleep(0.1)
 
        self.saveHDF5(cal_data, "PSGcalibration")
        self.homeAll()
